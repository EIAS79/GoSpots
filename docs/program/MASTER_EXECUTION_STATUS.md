...
| 1 | Tenancy, auth, permissions, integrity | Existing kernel retained; v2 delta adds complete venue role/permission catalog, business-day boundaries, money allocation, immutable contextual audit and correlated consumer-idempotent events. | PR and post-merge exact-head CI passed, including unit/property/security/integrity, clean/upgrade migration, browser, web and Edge suites. | `20260814090000_phase1_platform_kernel_v2` is expand-only and passes clean and representative historical-data upgrade assertions. | Vercel production revision, public web/API readiness and runtime-error checks pass; Render revision/migration-log proof and authenticated production settings/staff/audit smoke require provider/account access. | `SOFTWARE_DONE / BLOCKED_EXTERNAL` — no repository work remains; see `docs/acceptance/phase-1-platform-kernel-v2.md`. |
