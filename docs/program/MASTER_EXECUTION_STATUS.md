...
| 9 | Customers, membership, loyalty, packages, stored value | Phase 9 extends the canonical Growth domain with anonymous-first customer handling, consent provenance/preferences, membership lifecycle and usage ledgers, loyalty policy/expiry/refund reversal, prepaid package ledgers, stored-value policy/transfer/reconciliation, promotion usage evidence and a token-hash customer portal. Legacy Growth mutation paths are sealed through the shared Phase 1 idempotency/permission primitives instead of creating parallel authorities. | Dedicated Phase 9 rules, promotion stacking/time-window tests, real-PostgreSQL operational concurrency pilot, portal profile/history assertion, refund/reversal assertion, reconciliation assertion and permanent Chromium customer-portal E2E all pass on the final PR head; repository-wide and regression workflows are green. | `20260818100000_phase9_customer_value_completion` is expand-only with RLS/check constraints; dedicated CI proves the full clean chain and representative pre-Phase-9 upgrade preserving customer consent, loyalty and stored-value liability. | PR #69 was guarded-merged from exact green head `3c0e3b861d49780df06d646f4f00731b4c6165af` to `main` revision `c23167ff348b6afe27b69e2b53a615fcc2c28a9d`; exact revision deployed successfully to Vercel production, `www.gospots.pl` and `/customer/[token]` return HTTP 200, and post-deploy runtime/error-log checks are clean. | `ACCEPTED` — Gate P9 passes; see `docs/acceptance/phase9-customer-value.md`. |
...
| 12 | Offline-first, Edge Hub, hardware continuity | Phase 12 now has explicit offline certification policy, venue/operator cache, durable Edge command envelopes, deterministic sync/conflict/dead-letter handling, canonical cash replay, restart-safe print acknowledgement, scanner/display/drawer/KDS continuity and a persisted full-outage drill. | Exact implementation head `c42df4e3f14a93cb6564dd645ec0ce60df6fe1f9` passed all required API/web/Edge/browser/regression CI, including the dedicated hard-outage drill and persisted-state assertions. | No new PostgreSQL migration; additive Edge SQLite continuity tables only. Clean PostgreSQL 17 and representative historical upgrade migration gates passed. | PR #74 merged as `98d8dce46299a560bd8bfc499ee0b153b9c182be`; Render deployment `dep-da2dddk9v7es73c5gdt0` is live on that exact revision with clean migration/startup and new Edge routes mapped. Physical hardware matrix remains unperformed, and Vercel exact-main deployment is blocked by `upgradeToPro=build-rate-limit`. | `BLOCKED_EXTERNAL` — all executable software work is complete; Gate P12 requires representative physical hardware/outage evidence and exact-revision web deployment after the Vercel quota gate clears. See `docs/acceptance/phase-12-final-status.md`. |
...
| 17 | Pilot, certification, go-live | Phase 17 adds one release-certification gate, an eight-table realistic billiard pilot profile, release/runbook contracts, CSRF-safe identity-reset E2E support, and hardened narrow-screen release controls without creating a second business or financial authority. | Verified implementation head `e248239628e6d5f4770b6f881c3189ce6bf56624` passed the full permanent Playwright suite with flaky retries treated as failures, canonical persisted-state assertions, repository CI, Phase 3/4/7/8/9/10/11/13/16 regressions, Edge hard-outage and standalone boundary checks. | No Phase 17 Prisma migration. Clean PostgreSQL 17 migration deployment, representative historical upgrade, Prisma validation and independent logical backup/restore passed on the verified implementation head. | Executable software proof is complete. Gate P17 still requires real terminal/fiscal/KSeF certification, marketed physical hardware/Edge/KDS/inventory evidence, Polish accounting/legal validation, a full design-partner venue day without a shadow spreadsheet/POS, and exact-revision web production deployment proof after the Vercel release gate clears. | `SOFTWARE_DONE / BLOCKED_EXTERNAL` — see `docs/acceptance/phase-17-pilot-certification-go-live-v2.md`. |
