...
export class Phase13Service {
  ...
  async organizationOverview(actor: JwtAccessPayload, organizationId: string) {
    return this.withOrgBypass(actor, organizationId, async () => {
      const links = await this.prisma.organizationShop.findMany({
        where: { organizationId }, orderBy: [{ sortOrder: 'asc' }, { createdAt: 'asc' }],
      });
      ...
      const [shops, devices, transferCounts, purchaseCounts] = await Promise.all([
        this.prisma.shop.findMany({
          ...
          select: {
            ...
            subscription: { select: {
              tier: true, status: true, packId: true, trialEndsAt: true, currentPeriodEnd: true, staffSeatQuantity: true,
              billingSubscription: { select: { provider: true, planId: true, canonicalStatus: true, gracePeriodEndsAt: true, currentPeriodEnd: true } },
            } },
          },
        }),
        this.prisma.device.findMany({
          where: { shopId: { in: shopIds } },
          select: { id: true, shopId: true, label: true, type: true, status: true, claimState: true, lastSeenAt: true, softwareVersion: true },
        }),
      ...
      return {
        ...
        venues: links.map((link) => {
          ...
          return {
            ...
            inheritance: { inherited: link.inheritedSettings, override: link.overrideSettings },
            subscription: shop?.subscription ?? null,
            devices: {
              total: venueDevices.length,
              active: venueDevices.filter((device) => String(device.status) === 'ACTIVE').length,
              disabled: venueDevices.filter((device) => String(device.status) === 'DISABLED').length,
              items: venueDevices,
            },
            purchasing: Object.fromEntries(purchaseCounts.filter((row) => row.shopId === link.shopId).map((row) => [row.status, row._count._all])),
          };
        }),
  ...
  async listTransfers(actor: JwtAccessPayload, organizationId: string) {
    return this.withOrgBypass(actor, organizationId, () => this.prisma.organizationInventoryTransfer.findMany({
      where: { organizationId }, orderBy: { transferredAt: 'desc' }, take: 200,
    }));
  }
  ...
  async createTransfer(actor: JwtAccessPayload, organizationId: string, dto: CreateOrganizationInventoryTransferDto) {
    ...
    const requestHash = hashRequest({ ...dto, idempotencyKey: undefined });
    return this.withOrgBypass(actor, organizationId, async () => {
      ...
      const linked = await this.prisma.organizationShop.count({
      ...
      const created = await this.prisma.$transaction(async (tx) => {
        await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${`org-transfer:${dto.sourceShopId}:${dto.sourceStockItemId}:${dto.sourceLocationId}`}))`;
        const balance = await tx.stockMovement.aggregate({
          where: { shopId: dto.sourceShopId, stockItemId: dto.sourceStockItemId, locationId: dto.sourceLocationId }, _sum: { quantityMilli: true },
        });
        ...
        let transfer;
        try {
          transfer = await tx.organizationInventoryTransfer.create({ data: {
            organizationId, sourceShopId: dto.sourceShopId, destinationShopId: dto.destinationShopId,
            sourceStockItemId: dto.sourceStockItemId, destinationStockItemId: dto.destinationStockItemId,
            sourceLocationId: dto.sourceLocationId, destinationLocationId: dto.destinationLocationId,
            quantityMilli: dto.quantityMilli, unitCostMinor: sourceItem.weightedAverageCostMinor,
            idempotencyKey: dto.idempotencyKey, requestHash, requestedById: actor.sub, note: dto.note?.trim() || null,
          } });
        } catch (error) {
          if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
            throw new ConflictException('Concurrent idempotency collision; retry the same request');
          }
          throw error;
        }
        await tx.stockMovement.create({ data: {
          shopId: dto.sourceShopId, stockItemId: dto.sourceStockItemId, locationId: dto.sourceLocationId,
          kind: 'TRANSFER_OUT_ORGANIZATION', quantityMilli: -dto.quantityMilli,
          ...
          referenceType: 'ORGANIZATION_TRANSFER', referenceId: transfer.id,
          movementKey: `org-transfer:${transfer.id}:out`, note: dto.note?.trim() || null, actorUserId: actor.sub,
        } });
        return transfer;
      });
      await Promise.all([
    ...
    return this.withOrgBypass(actor, organizationId, async () => {
      ...
      const updated = await this.prisma.$transaction(async (tx) => {
        ...
        if (dto.receivedMilli > 0) await tx.stockMovement.create({ data: {
          shopId: current.destinationShopId, stockItemId: current.destinationStockItemId, locationId: current.destinationLocationId,
          kind: 'TRANSFER_IN_ORGANIZATION', quantityMilli: dto.receivedMilli,
          unitCostMinor: current.unitCostMinor, totalCostMinor: Math.trunc((dto.receivedMilli * current.unitCostMinor) / 1000),
          referenceType: 'ORGANIZATION_TRANSFER', referenceId: current.id,
        ...
        return tx.organizationInventoryTransfer.update({ where: { id: current.id }, data: {
          receivedMilli: dto.receivedMilli, damagedMilli: dto.damagedMilli, missingMilli: dto.missingMilli,
          status: OrganizationInventoryTransferStatus.RECEIVED, receivedById: actor.sub, receivedAt: new Date(), note: dto.note?.trim() || current.note,
        } });
      });
    ...
    return this.withOrgBypass(actor, organizationId, async () => {
      ...
      const order = await this.prisma.$transaction(async (tx) => {
        const created = await tx.purchaseOrder.create({ data: {
          shopId: dto.destinationShopId, supplierId: dto.supplierId, locationId: dto.locationId,
          status: 'DRAFT', documentRef: dto.documentRef?.trim() || null, createdById: actor.sub,
        } });
        await tx.purchaseOrderLine.createMany({ data: dto.lines.map((line) => ({
          shopId: dto.destinationShopId, purchaseOrderId: created.id, stockItemId: line.stockItemId,
          orderedMilli: line.orderedMilli, unitCostMinor: line.unitCostMinor,
        })) });
        return created;
      });
  ...
  async previewImport(actor: JwtAccessPayload, dto: ImportPreviewDto) {
    ...
    const job = await this.prisma.dataImportJob.create({ data: {
      shopId, kind: dto.kind, status: errors.length ? DataImportStatus.REJECTED : DataImportStatus.PREVIEW_READY,
      sourceHash, rowCount: rows.length, rows: rows as Prisma.InputJsonValue,
      preview: { valid: errors.length === 0, errors: errors.slice(0, 100), rowCount: rows.length, requiredHeaders: requiredHeaders(dto.kind) },
  ...
  async commitImport(actor: JwtAccessPayload, jobId: string) {
      ...
      for (let index = 0; index < rows.length; index += 1) {
        ...
        if (job.kind === DataImportKind.PRODUCTS) {
          await tx.menuItem.create({ data: {
            shopId, name: row.name.trim(), price: new Prisma.Decimal(row.price), sku: row.sku?.trim() || null,
            barcode: row.barcode?.trim() || null,
            kind: Object.values(CatalogItemKind).includes(row.kind as CatalogItemKind) ? row.kind as CatalogItemKind : CatalogItemKind.PRODUCT,
          ...
          await tx.resource.create({ data: {
            ...
            type: Object.values(ResourceType).includes(row.type as ResourceType) ? row.type as ResourceType : ResourceType.OTHER,
            hourlyRate: new Prisma.Decimal(row.hourlyRate || '0'),
            configurationState: Object.values(ResourceConfigurationState).includes(row.configurationState as ResourceConfigurationState) ? row.configurationState as ResourceConfigurationState : ResourceConfigurationState.ENABLED,
          } });
          ...
          await tx.stockMovement.create({ data: {
            shopId, stockItemId: item.id, locationId: location.id, kind: 'OPENING_BALANCE', quantityMilli,
            unitCostMinor: item.weightedAverageCostMinor, totalCostMinor: Math.trunc((quantityMilli * item.weightedAverageCostMinor) / 1000),
            referenceType: 'DATA_IMPORT', referenceId: job.id, movementKey: `import:${job.id}:${index}`,
    ...
    await this.audit.record(actor, { section: 'system', action: 'data_import.committed', summary: `Committed ${job.kind} import`, meta: { importJobId: job.id, rowCount: rows.length, sourceHash: job.sourceHash } });
    return committed;
  }
  ...
  async exportCsv(actor: JwtAccessPayload, kind: DataImportKind) {
    ...
    if (kind === DataImportKind.PRODUCTS) {
      ...
      const [memberships, customers, tiers] = await Promise.all([
        this.prisma.customerMembership.findMany({ where: { shopId } }), this.prisma.customerProfile.findMany({ where: { shopId } }), this.prisma.membershipTier.findMany({ where: { shopId } }),
      ]);
      ...
      const [movements, items, locations] = await Promise.all([
        this.prisma.stockMovement.findMany({ where: { shopId, kind: 'OPENING_BALANCE' }, orderBy: { occurredAt: 'asc' } }), this.prisma.stockItem.findMany({ where: { shopId } }), this.prisma.inventoryLocation.findMany({ where: { shopId } }),
      ]);
  ...
  async systemDiagnostics(actor: JwtAccessPayload, shopId: string) {
    ...
    const [memberships, devices, deadJobs, deadWebhooks, importJobs] = await Promise.all([
      this.prisma.membership.count({ where: { shopId } }), this.prisma.device.count({ where: { shopId } }),
      this.prisma.integrationJob.count({ where: { shopId, status: 'DEAD' } }), this.prisma.webhookDelivery.count({ where: { shopId, status: 'DEAD' } }), this.prisma.dataImportJob.count({ where: { shopId } }),
    ]);
  ...
  async updateSystemSubscription(actor: JwtAccessPayload, shopId: string, dto: SystemSubscriptionUpdateDto) {
    ...
    const parseDate = (value: string | null | undefined, label: string) => {
      ...
      const date = new Date(value); if (Number.isNaN(date.getTime())) throw new BadRequestException(`${label} must be an ISO date`); return date;
    };
    const updated = await this.prisma.subscription.update({ where: { shopId }, data: {
      tier: dto.tier, status: dto.status, packId: dto.packId?.trim(), staffSeatQuantity: dto.staffSeatQuantity,
      trialEndsAt: parseDate(dto.trialEndsAt, 'trialEndsAt'), currentPeriodEnd: parseDate(dto.currentPeriodEnd, 'currentPeriodEnd'),
    } });
    await this.audit.recordForShop(shopId, {
      section: 'subscription', action: 'system_admin.subscription.updated', summary: 'System admin updated manual subscription entitlement', actorName: actor.email,
      previousState: { tier: current.tier, status: current.status, packId: current.packId, staffSeatQuantity: current.staffSeatQuantity },
      newState: { tier: updated.tier, status: updated.status, packId: updated.packId, staffSeatQuantity: updated.staffSeatQuantity }, reason: 'System admin plan/subscription management',
    });
    return updated;
  }
