...
describe('Integration platform reliability', () => {
  const actor = {
    sub: 'user-1', shopId: 'shop-1', shopRole: 'OWNER', perms: '*', email: 'owner@example.com',
  } as any;
  ...
  function setup() {
    ...
    const prisma: any = {
      ...
      integrationJob: {
        create: jest.fn(), findUnique: jest.fn(), findMany: jest.fn(), updateMany: jest.fn(), update: jest.fn(),
      },
      webhookDelivery: { findMany: jest.fn(), updateMany: jest.fn(), update: jest.fn() },
    ...
    const secretBox = { encrypt: jest.fn(), decrypt: jest.fn().mockReturnValue({ webhookSecret: 'secret' }) } as any;
    const registry = { get: jest.fn().mockReturnValue(connector), list: jest.fn().mockReturnValue([]) } as any;
    const service = new IntegrationsService(prisma, flags, audit, secretBox, registry);
  ...
  it('atomically claims a durable job before connector execution', async () => {
    ...
    prisma.integrationJob.findUnique.mockResolvedValue({
      id: 'job-1', shopId: 'shop-1', installationId: 'install-1', jobType: 'session.charge',
      idempotencyKey: 'idem-1', payload: { amount: '10.00' }, correlationId: null,
      attemptCount: 0, maxAttempts: 5,
      installation: { id: 'install-1', provider: 'demo', status: ConnectorInstallationStatus.ACTIVE, config: null },
    });
  ...
  it('blocks private webhook targets before persistence', async () => {
    ...
    await expect(service.createWebhookEndpoint(actor, {
      name: 'bad', url: 'https://127.0.0.1/hook', eventTypes: ['*'],
  ...
  it('enqueues only against an installation owned by the same shop', async () => {
    ...
    await expect(service.enqueueForShop('shop-1', 'foreign-install', {
      direction: IntegrationDirection.OUTBOUND,
      jobType: 'session.charge', idempotencyKey: 'idem-1', payload: {}, maxAttempts: 3,
    }, null)).rejects.toThrow('Active connector installation not found');
    expect(prisma.integrationJob.create).not.toHaveBeenCalled();
  });
});

describe('GoPOS connector boundary', () => {
  const connector = new GoPosIntegrationConnector();

  it('advertises licensed API requirement and remains fail-closed', async () => {
    expect(connector.capabilities()).toEqual(expect.objectContaining({ requiresLicensedApi: true }));
    await expect(connector.execute({
      shopId: 'shop-1', installationId: 'i-1', config: {}, secrets: null,
    }, {
      id: 'j-1', jobType: 'session.charge', idempotencyKey: 'idem-1', payload: {}, correlationId: null,
    })).rejects.toBeInstanceOf(ServiceUnavailableException);
  });

  it('does not pretend configured contract metadata is a live health check', async () => {
    await expect(connector.health({
      shopId: 'shop-1', installationId: 'i-1', config: { apiContractVersion: 'licensed-v1' }, secrets: {},
