...
function edgeMetadata(metadata: unknown): EdgeMetadata {
  return asObject(asObject(metadata).edge) as EdgeMetadata;
}

function withEdgeMetadata(metadata: unknown, edge: EdgeMetadata): Prisma.InputJsonValue {
  const cleanEdge = Object.fromEntries(
    Object.entries(edge).filter(([, value]) => value !== undefined),
  );
...
export class EdgeHubService {
  ...
  async createProvisioningToken(actor: JwtAccessPayload, deviceId: string) {
    ...
    const device = await this.prisma.device.findFirst({
      where: { id: deviceId, shopId, type: DeviceType.EDGE_HUB, status: DeviceStatus.ACTIVE },
    });
    ...
    await this.prisma.device.update({
      where: { id: device.id },
      data: {
        metadata: withEdgeMetadata(device.metadata, {
          ...edge,
          provisionTokenHash: sha256(token),
          provisionExpiresAt: expiresAt.toISOString(),
          provisionUsedAt: undefined,
    ...
    await this.audit.record(actor, {
      section: 'system',
      action: 'edge.provision.created',
      summary: `Created one-time Edge Hub provisioning token for ${device.label}`,
      meta: { deviceId: device.id, expiresAt: expiresAt.toISOString() },
    });
  ...
  async register(dto: RegisterEdgeHubDto) {
    ...
    let key;
    try {
      key = createPublicKey(dto.publicKeyPem);
    } catch {
      throw new BadRequestException('Invalid Edge Hub public key');
    }
  ...
  async authenticateSignedRequest(
    ...
    try {
      valid = verify(
        ...
        createPublicKey(edge.publicKeyPem),
    ...
    try {
      ...
      if ((error as { code?: string })?.code === 'P2002') {
        throw new UnauthorizedException('Edge Hub nonce was already used');
      }
      throw error;
    }
    return { device, edge };
  ...
  async heartbeat(
    ...
    const { device, edge } = await this.authenticateSignedRequest(
      headers,
      'POST',
      '/edge-hub/cloud/heartbeat',
      dto,
    );
    const now = new Date();
    await this.prisma.device.update({
      ...
      data: {
        lastSeenAt: now,
        metadata: withEdgeMetadata(device.metadata, { ...edge, version: dto.version }),
      },
    });
  ...
  async replay(
    ...
    const { device } = await this.authenticateSignedRequest(
      headers,
      'POST',
      '/edge-hub/cloud/replay',
      dto,
    );
    const safeDeviceId = `edge:${device.id}:${dto.deviceId}`.slice(0, 120);
    return this.offline.applyEdgeOperation(device.shopId, device.id, {
      ...dto,
