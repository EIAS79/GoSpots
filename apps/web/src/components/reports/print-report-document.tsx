...
@media print {
  ...
  .gs-print-report {
    ...
    margin: 0 !important;
    padding: 10mm 11mm 12mm !important;
...
function parseNumericCell(text: string): number | null {
  ...
  const negative = /^\s*\(/.test(source) || source.includes("-");
...
function parseReportTable(table: HTMLTableElement): ParsedReportTable | null {
  ...
  for (let index = 1; index < headers.length; index += 1) {
    const parsed = cells.map((row) => parseNumericCell(textOf(row[index] ?? null)));
...
function drawGrid(svg: SVGSVGElement, max: number, left = 54, right = 16, top = 15, bottom = 38) {
...
function lineChart(parsed: ParsedReportTable, column = parsed.numericColumns[0]) {
  const svg = baseSvg();
  const values = column.values;
  const max = Math.max(1, ...values.map((value) => Math.abs(value)));
  const area = drawGrid(svg, max);
  const points = values.map((value, index) => {
    const x = area.left + (values.length <= 1 ? area.width / 2 : (area.width * index) / (values.length - 1));
    const y = area.top + area.height - (Math.max(0, value) / max) * area.height;
    return { x, y };
  });
  ...
  for (const point of points) {
    svg.appendChild(svgEl("circle", { cx: point.x, cy: point.y, r: 3.2, fill: "#ffffff", stroke: CHART_COLORS[0], "stroke-width": 2 }));
  }
  ...
  appendText(svg, area.left, 11, column.header, { fill: "#334155", "font-size": 10, "font-weight": 700 });
  return svg;
}
...
function groupedBarChart(parsed: ParsedReportTable) {
  ...
  const max = Math.max(1, ...columns.flatMap((column) => column.values.map((value) => Math.abs(value))));
  ...
  columns.forEach((column, seriesIndex) => {
    column.values.forEach((value, index) => {
      ...
      const x = area.left + groupWidth * index + (groupWidth - usable) / 2 + barWidth * seriesIndex;
  ...
  parsed.labels.forEach((label, index) => {
    if (index % stride !== 0 && index !== parsed.labels.length - 1) return;
    appendText(svg, area.left + groupWidth * index + groupWidth / 2, 207, label.slice(0, 13), {
      fill: "#64748b",
      "font-size": 8.5,
      "text-anchor": "middle",
    });
  ...
  columns.forEach((column, index) => {
    svg.appendChild(svgEl("rect", { x: legendX, y: 8, width: 9, height: 9, rx: 1, fill: CHART_COLORS[index] }));
    appendText(svg, legendX + 13, 16, column.header.slice(0, 18), { fill: "#475569", "font-size": 9 });
    legendX += Math.min(165, 35 + column.header.length * 6);
  });
  return svg;
}

function horizontalBarChart(parsed: ParsedReportTable, column = parsed.numericColumns[parsed.numericColumns.length - 1]) {
  ...
  const svg = svgEl("svg", { viewBox: `0 0 760 ${height}`, role: "img", "aria-hidden": "true" });
  svg.style.fontFamily = "Arial, Helvetica, sans-serif";
  appendText(svg, 175, 16, column.header, { fill: "#334155", "font-size": 10, "font-weight": 700 });
  labels.forEach((label, index) => {
    const y = 31 + index * 22;
    appendText(svg, 165, y + 10, label.slice(0, 27), { fill: "#475569", "font-size": 9.5, "text-anchor": "end" });
    svg.appendChild(svgEl("rect", { x: 175, y, width: 520, height: 13, rx: 2, fill: "#edf2f7" }));
    svg.appendChild(svgEl("rect", {
      x: 175,
      y,
      ...
      height: 13,
      rx: 2,
      fill: (values[index] ?? 0) < 0 ? "#e11d48" : CHART_COLORS[index % CHART_COLORS.length],
    }));
    appendText(svg, 705, y + 10, compactNumber(values[index] ?? 0), { fill: "#334155", "font-size": 9, "text-anchor": "start" });
  });
  return svg;
}
...
function createAutoChart(parsed: ParsedReportTable): SVGElement | null {
  ...
  const hasQuantity = lowerHeaders.some((header) => /quantity|qty|ilość|ilosc/.test(header));
  const hasTotal = lowerHeaders.some((header) => /total|razem|łącznie|lacznie/.test(header));
  if (parsed.numericColumns.length === 1) {
    return parsed.labels.length >= 5
      ? lineChart(parsed)
      : horizontalBarChart(parsed);
  }
  if (hasQuantity || parsed.labels.length <= 4) {
    return horizontalBarChart(parsed);
  }
  if (parsed.numericColumns.length >= 4 || parsed.labels.length > 16) {
    const totalColumn = hasTotal
      ? [...parsed.numericColumns].reverse().find((column) => /total|razem|łącznie|lacznie/.test(column.header.toLowerCase()))
      : undefined;
    if (totalColumn) return lineChart(parsed, totalColumn);
    const synthetic: ParsedReportTable["numericColumns"][number] = {
      index: -1,
      header: "Total",
      values: parsed.labels.map((_, rowIndex) => parsed.numericColumns.reduce((sum, column) => sum + (column.values[rowIndex] ?? 0), 0)),
    };
...
function enhanceReportWithCharts(root: HTMLElement) {
  root.querySelectorAll(".gs-report-auto-chart").forEach((node) => node.remove());
  root.querySelectorAll<HTMLTableElement>(".gs-report-section .gs-report-table").forEach((table) => {
...
export function PrintReportDocument({
  ...
  useLayoutEffect(() => {
    if (!mounted) return;
    const root = document.querySelector<HTMLElement>(`.gs-print-report[data-report-title="${CSS.escape(title)}"]`);
  ...
  const labels = isPolish
    ? { period: "Okres", currency: "Waluta", generated: "Wygenerowano", footer: "Raport wygenerowany przez GoSpots" }
    : { period: "Period", currency: "Currency", generated: "Generated", footer: "Report generated by GoSpots" };

  if (!mounted) return null;

  return createPortal(
    <div className="gs-print-report" data-report-title={title} aria-hidden="true">
    ...
    <section className="gs-report-kpis">
      {items.map((item) => (
        <div className="gs-report-kpi" key={item.label}>
          ...
          {item.note ? <span className="gs-report-kpi-note">{item.note}</span> : null}
        </div>
      ))}
    </section>
  );
