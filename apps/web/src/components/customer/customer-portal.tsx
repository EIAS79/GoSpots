...
export function CustomerPortal({ token }: { token: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSnapshot(null);
      setError(reason instanceof Error ? reason.message : "Customer portal unavailable.");
    } finally {
      setLoading(false);
    }
  }, [endpoint]);
  ...
  async function changeConsent(granted: boolean) {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Consent preference could not be saved.");
      setSnapshot({
        ...snapshot,
        customer: {
          ...snapshot.customer,
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-5xl px-6 py-12">
        <header className="mb-8">
          <p className="text-sm font-medium text-muted-foreground">GoSpots customer portal</p>
          <h1 className="mt-2 text-3xl font-semibold tracking-tight">{displayName}</h1>
          <p className="mt-2 text-muted-foreground">
            Memberships, benefits, reservations, value balances and documents from your venue account.
          </p>
        </header>

        {error ? (
          <div className="mb-6 rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-sm" role="alert">
            {error}
          </div>
        ) : null}
          ...
          <section className="rounded-2xl border border-border bg-card p-6">
            ...
            {snapshot.membership ? (
              <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
                ...
                <dd className="font-medium">{snapshot.membership.effectiveStatus}</dd>
              ...
              <p className="mt-4 text-sm text-muted-foreground">No membership is currently linked.</p>
            )}
          </section>

          <section className="rounded-2xl border border-border bg-card p-6">
            <h2 className="text-lg font-semibold">Loyalty</h2>
            <p className="mt-4 text-3xl font-semibold">{snapshot.loyalty.balance}</p>
            ...
            {snapshot.packages.length ? (
              <ul className="mt-4 space-y-3">
                {snapshot.packages.map(({ account, balanceUnits }) => (
                  <li key={account.id} className="flex items-center justify-between gap-4 rounded-xl border border-border p-3">
                    <span>
                      <span className="block font-medium">{account.unitKind}</span>
              ...
              <p className="mt-4 text-sm text-muted-foreground">No prepaid package balance.</p>
            )}
          </section>
            ...
            {snapshot.storedValue.length ? (
              <ul className="mt-4 space-y-3">
                {snapshot.storedValue.map(({ account, balanceMinor }) => (
                  <li key={account.id} className="flex items-center justify-between rounded-xl border border-border p-3">
                    <span className="text-sm text-muted-foreground">{account.status}</span>
                    <strong>{formatMinor(balanceMinor, account.currency)}</strong>
                  </li>
                ))}
              </ul>
            ) : (
              <p className="mt-4 text-sm text-muted-foreground">No stored-value balance.</p>
            )}
          </section>
        </div>
        ...
        <section className="mt-6 rounded-2xl border border-border bg-card p-6">
          ...
          {snapshot.upcomingReservations.length ? (
            <ul className="mt-4 divide-y divide-border">
              {snapshot.upcomingReservations.map((reservation) => (
                <li key={reservation.id} className="flex flex-wrap items-center justify-between gap-3 py-3">
                  <span>
                    <span className="block font-medium">{formatDate(reservation.startsAt)}</span>
            ...
            <p className="mt-4 text-sm text-muted-foreground">No upcoming reservations.</p>
          )}
        </section>
                ...
                <li key={document.id} className="flex flex-wrap items-center justify-between gap-3 py-3 text-sm">
                  <span>
                    <span className="block font-medium">
                      {document.kind} {document.documentNumber ? `#${document.documentNumber}` : ""}
                    </span>
                    <span className="text-muted-foreground">{formatDate(document.issueDate)} · {document.state}</span>
                  </span>
                  <strong>{Number(document.grossAmount).toFixed(2)} {document.currency}</strong>
                </li>
              ))}
            </ul>
          ) : (
            <p className="mt-4 text-sm text-muted-foreground">No customer documents are linked yet.</p>
          )}
        </section>
        ...
        <section className="mt-6 rounded-2xl border border-border bg-card p-6">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
            Marketing consent is separate from the operational records the venue must retain for legitimate business and legal purposes.
          </p>
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <span className="text-sm">
              Marketing: <strong>{snapshot.customer.marketingConsent ? "Allowed" : "Not allowed"}</strong>
            </span>
            <button
              type="button"
              disabled={savingConsent}
              onClick={() => void changeConsent(!snapshot.customer.marketingConsent)}
